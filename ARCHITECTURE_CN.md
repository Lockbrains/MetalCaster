...
### 4.5 为什么用 NSViewRepresentable 而不是 UIViewRepresentable？

这是 **macOS** 应用。在 macOS 上，SwiftUI 底层使用 AppKit，因此需要遵循 `NSViewRepresentable`（而非 `UIViewRepresentable`，后者仅用于 iOS/iPadOS）。包装的视图是 `MTKView`，基于 `CAMetalLayer` 提供可绘制的渲染目标。
