...
extension UTType {
    static let sdfCanvas = UTType(exportedAs: "com.metalcaster.sdfcanvas")
}
...
public indirect enum SDFNode: Codable, Sendable, Equatable {
    ...
    /// Whether the subtree contains the given node id.
    public func contains(id target: UUID) -> Bool {
        find(id: target) != nil
    }

    /// Returns a deep copy with freshly regenerated node IDs.
    public func regeneratedIDs() -> SDFNode {
        switch self {
        case .sphere(_, let radius):
            return .sphere(id: UUID(), radius: radius)
        case .box(_, let size):
            return .box(id: UUID(), size: size)
        case .roundedBox(_, let size, let radius):
            return .roundedBox(id: UUID(), size: size, radius: radius)
        case .cylinder(_, let radius, let height):
            return .cylinder(id: UUID(), radius: radius, height: height)
        case .torus(_, let majorRadius, let minorRadius):
            return .torus(id: UUID(), majorRadius: majorRadius, minorRadius: minorRadius)
        case .capsule(_, let radius, let height):
            return .capsule(id: UUID(), radius: radius, height: height)
        case .cone(_, let radius, let height):
            return .cone(id: UUID(), radius: radius, height: height)

        case .union(_, let a, let b):
            return .union(id: UUID(), a.regeneratedIDs(), b.regeneratedIDs())
        case .subtraction(_, let a, let b):
            return .subtraction(id: UUID(), a.regeneratedIDs(), b.regeneratedIDs())
        case .intersection(_, let a, let b):
            return .intersection(id: UUID(), a.regeneratedIDs(), b.regeneratedIDs())
        case .smoothUnion(_, let a, let b, let k):
            return .smoothUnion(id: UUID(), a.regeneratedIDs(), b.regeneratedIDs(), k: k)
        case .smoothSubtraction(_, let a, let b, let k):
            return .smoothSubtraction(id: UUID(), a.regeneratedIDs(), b.regeneratedIDs(), k: k)
        case .smoothIntersection(_, let a, let b, let k):
            return .smoothIntersection(id: UUID(), a.regeneratedIDs(), b.regeneratedIDs(), k: k)

        case .transform(_, let child, let position, let rotation, let scale):
            return .transform(
                id: UUID(),
                child: child.regeneratedIDs(),
                position: position,
                rotation: rotation,
                scale: scale
            )
        case .round(_, let child, let radius):
            return .round(id: UUID(), child: child.regeneratedIDs(), radius: radius)
        case .onion(_, let child, let thickness):
            return .onion(id: UUID(), child: child.regeneratedIDs(), thickness: thickness)
        case .twist(_, let child, let amount):
            return .twist(id: UUID(), child: child.regeneratedIDs(), amount: amount)
        case .bend(_, let child, let amount):
            return .bend(id: UUID(), child: child.regeneratedIDs(), amount: amount)
        case .elongate(_, let child, let h):
            return .elongate(id: UUID(), child: child.regeneratedIDs(), h: h)
        case .repeatSpace(_, let child, let period):
            return .repeatSpace(id: UUID(), child: child.regeneratedIDs(), period: period)
        }
    }

    /// Inserts `newNode` under `target` in a non-destructive way.
    ///
    /// Since SDF operators have fixed arity, insertion appends by wrapping
    /// an existing child with `union(existing, newNode)` where needed.
    public func insertingAsChild(_ newNode: SDFNode, into target: UUID) -> SDFNode? {
        var didInsert = false
        let updated = insertingAsChildImpl(newNode, into: target, didInsert: &didInsert)
        return didInsert ? updated : nil
    }

    private func insertingAsChildImpl(_ newNode: SDFNode, into target: UUID, didInsert: inout Bool) -> SDFNode {
        if self.id == target {
            didInsert = true
            switch self {
            case .sphere, .box, .roundedBox, .cylinder, .torus, .capsule, .cone:
                // Primitive cannot own children: promote to a union parent.
                return .union(id: UUID(), self, newNode)

            case .union(let id, let a, let b):
                return .union(id: id, a, .union(id: UUID(), b, newNode))
            case .subtraction(let id, let a, let b):
                return .subtraction(id: id, a, .union(id: UUID(), b, newNode))
            case .intersection(let id, let a, let b):
                return .intersection(id: id, a, .union(id: UUID(), b, newNode))
            case .smoothUnion(let id, let a, let b, let k):
                return .smoothUnion(id: id, a, .union(id: UUID(), b, newNode), k: k)
            case .smoothSubtraction(let id, let a, let b, let k):
                return .smoothSubtraction(id: id, a, .union(id: UUID(), b, newNode), k: k)
            case .smoothIntersection(let id, let a, let b, let k):
                return .smoothIntersection(id: id, a, .union(id: UUID(), b, newNode), k: k)

            case .transform(let id, let child, let pos, let rot, let scale):
                return .transform(
                    id: id,
                    child: .union(id: UUID(), child, newNode),
                    position: pos,
                    rotation: rot,
                    scale: scale
                )
            case .round(let id, let child, let radius):
                return .round(id: id, child: .union(id: UUID(), child, newNode), radius: radius)
            case .onion(let id, let child, let thickness):
                return .onion(id: id, child: .union(id: UUID(), child, newNode), thickness: thickness)
            case .twist(let id, let child, let amount):
                return .twist(id: id, child: .union(id: UUID(), child, newNode), amount: amount)
            case .bend(let id, let child, let amount):
                return .bend(id: id, child: .union(id: UUID(), child, newNode), amount: amount)
            case .elongate(let id, let child, let h):
                return .elongate(id: id, child: .union(id: UUID(), child, newNode), h: h)
            case .repeatSpace(let id, let child, let period):
                return .repeatSpace(id: id, child: .union(id: UUID(), child, newNode), period: period)
            }
        }
        ...
        case .union(let id, let a, let b):
            return .union(
                id: id,
                a.insertingAsChildImpl(newNode, into: target, didInsert: &didInsert),
                b.insertingAsChildImpl(newNode, into: target, didInsert: &didInsert)
            )
        case .subtraction(let id, let a, let b):
            return .subtraction(
                id: id,
        ...
        case .transform(let id, let child, let pos, let rot, let scale):
            return .transform(
                id: id,
                child: child.insertingAsChildImpl(newNode, into: target, didInsert: &didInsert),
                position: pos,
                rotation: rot,
                scale: scale
            )
...
extension NSNotification.Name {
    ...
    static let sdfCanvasUndo     = NSNotification.Name("sdfCanvasUndo")
    static let sdfCanvasRedo     = NSNotification.Name("sdfCanvasRedo")
    static let sdfCanvasCopy     = NSNotification.Name("sdfCanvasCopy")
    static let sdfCanvasPaste    = NSNotification.Name("sdfCanvasPaste")
}
